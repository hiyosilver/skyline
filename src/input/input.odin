...
get_input :: proc() -> RawInput {
	...
	for mouseButton in rl.MouseButton {
		...
		} else if rl.IsMouseButtonDown(rl.MouseButton(mouseButton)) &&
		   InputFlags.ChangedThisFrame not_in input.mouse_buttons[mouseButton] {
	...
	for key in rl.KeyboardKey {
		...
		} else if rl.IsKeyDown(rl.KeyboardKey(key)) &&
		   InputFlags.ChangedThisFrame not_in input.keys[key] {
...
is_mouse_button_pressed_this_frame :: proc(button: rl.MouseButton, input: ^RawInput) -> bool {
	return(
		InputFlags.Down in input.mouse_buttons[button] &&
		InputFlags.ChangedThisFrame in input.mouse_buttons[button] \
	)
}

is_mouse_button_released_this_frame :: proc(button: rl.MouseButton, input: ^RawInput) -> bool {
	return(
		InputFlags.Down not_in input.mouse_buttons[button] &&
		InputFlags.ChangedThisFrame in input.mouse_buttons[button] \
	)
}
