...
make_job_display :: proc(job: ^jobs.Job) -> JobDisplay {
	...
	widget.start_button = make_simple_button(
		.OnRelease,
		button_color,
		{80.0, 0.0},
		widget.button_label,
	...
	if job.ticks_needed > 0 {
		...
		for _ in 0 ..< job.ticks_needed {
			bar := make_loading_bar(0, 1.0, rl.YELLOW, rl.DARKGRAY, {segment_width, 8.0})

			box_add_child(widget.progress_box, bar)
		}
	}

	widget.root = make_panel(
		base_color,
		{320.0, 120.0},
		make_margin(
			16,
			...
			make_box(
				...
				make_box(
					...
					make_box(.Vertical, .Start, .End, 4, widget.start_button, buyin_pill),
				),
				widget.ticks_label,
				widget.progress_box,
			),
		...
		make_margin(
			...
			make_box(
				...
				make_box(
					...
					make_pill(rl.RAYWHITE, {}, widget.salary_label),
				),
				make_box(
					.Vertical,
					.Start,
...
update_crew_member_display :: proc(
	widget: ^CrewMemberDisplay,
	cm: ^crew.CrewMember,
	tick_timer: f32,
	tick_speed: f32,
	...
	if cm.base_salary > 0.0 {
		if cm.base_salary_illegitimate > 0.0 {
			...
			label_set_text(
				widget.salary_label,
				fmt.tprintf("%s $", global.format_float_thousands(cm.base_salary, 2)),
			)
		}
	} else {
		label_set_text(
			widget.salary_label,
			fmt.tprintf("%s ₴", global.format_float_thousands(cm.base_salary_illegitimate, 2)),
		)
	}
...
make_stock_window :: proc(market: ^stocks.Market) -> StockWindow {
	...
	widget.buy_button = make_simple_button(
		.OnRelease,
		rl.GRAY,
		{100.0, 0.0},
		make_label("Buy 1", global.font, 24, rl.BLACK),
	)
	widget.sell_button = make_simple_button(
		.OnRelease,
		rl.GRAY,
		{100.0, 0.0},
	...
	widget.buy_all_button = make_simple_button(
		.OnRelease,
		rl.GRAY,
		{100.0, 0.0},
	...
	widget.sell_all_button = make_simple_button(
		.OnRelease,
		rl.GRAY,
		{100.0, 0.0},
		...
		make_margin(
			...
			make_box(
				...
				make_box(
					...
					make_box(.Horizontal, .Start, .Fill, 8, widget.buy_button, widget.sell_button),
					make_box(
						.Horizontal,
						.Start,
						.Fill,
