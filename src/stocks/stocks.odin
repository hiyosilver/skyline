...
create_market :: proc() -> Market {
	...
	companies[103] = create_company(103, "Nexus Telecom", "NEX", "National wireless and internet provider", .CommunicationServices, 45, 2_500_000_000, 0.98, Arch_UtilityGiant)
	companies[104] = create_company(104, "Streamline", "STRM", "Video streaming and content production", .CommunicationServices, 8, 120_000_000, 0.60, Arch_GrowthAggressive)
	...
	companies[203] = create_company(204, "Velocity Sports", "VELO", "Athletic footwear and apparel", .ConsumerDiscretionary, 30, 450_000_000, 0.85, Arch_BigTechLeader)
	...
	companies[302] = create_company(302, "Sparkle Beverage", "FIZZ", "Soft drinks and snacks", .ConsumerStaples, 95, 3_000_000_000, 0.92, Arch_DividendAristocrat)
    companies[303] = create_company(303, "Hearth & Home", "HAH", "Cleaning supplies and paper products", .ConsumerStaples, 70, 600_000_000, 0.88, Arch_Distressed)

	//Energy
	companies[401] = create_company(401, "PetroMax", "PMAX", "Oil and gas", .Energy, 110, 1_150_000_000, 0.98, Arch_CommodityCyclical)
	companies[402] = create_company(402, "Helios Renewables", "HEL", "Solar panel manufacturing and farms", .Energy, 12, 150_000_000, 0.70, Arch_GrowthAggressive)
	...
	companies[502] = create_company(502, "Pinnacle Capital", "PIN", "Investment banking", .Financials, 12, 25_000_000, 0.8, Arch_GrowthAggressive)
	companies[503] = create_company(503, "PayFlow Systems", "PAY", "Global credit card processing", .Financials, 25, 900_000_000, 0.80, Arch_BigTechLeader)
    companies[504] = create_company(504, "Aegis Insurance", "AEGS", "Property and casualty insurance", .Financials, 110, 400_000_000, 0.95, Arch_FinancialAnchor)
	...
	companies[602] = create_company(602, "Helix Therapeutics", "HLX", "Gene editing research", .Healthcare, 4, 30_000_000, 0.40, Arch_Startup)

	//Industrial
	companies[701] = create_company(701, "Condor Industries", "CDI", "Aerospace", .Industrial, 35, 177_000_000, 0.8, Arch_BigTechLeader)
	companies[702] = create_company(702, "Arrow Logistics", "ARR", "Global package delivery", .Industrial, 40, 350_000_000, 0.90, Arch_CommodityCyclical)
    companies[703] = create_company(703, "Vanguard", "VGD", "Military aircraft and systems", .Industrial, 70, 200_000_000, 0.80, Arch_FinancialAnchor)
	...
	companies[802] = create_company(803, "Sentinel Cyber", "LOCK", "Enterprise network security", .InformationTechnology, 9, 80_000_000, 0.50, Arch_GrowthAggressive)
	...
	companies[902] = create_company(902, "Titan Steel", "TITN", "Industrial steel production", .Materials, 85, 250_000_000, 0.85, Arch_CommodityCyclical)
	...
	companies[1102] = create_company(1102, "AquaSource", "AQUA", "Municipal water treatment", .Utilities, 75, 150_000_000, 0.95, Arch_UtilityGiant)
...
update_market_tick :: proc(market: ^Market) {
    ...
    for _, &company in market.companies {
    	...
    	company.perceived_eps = math.lerp(company.perceived_eps, company.earnings_per_share, market_reaction_speed)
    	company.perceived_rps = math.lerp(company.perceived_rps, company.revenue_per_share, market_reaction_speed)
...
update_market_year :: proc(market: ^Market) {
    ...
    for _, &company in market.companies {
        ...
        if math.abs(company.eps_start_of_year) > 0.01 {
            eps_delta_percent = (company.earnings_per_share - company.eps_start_of_year) / math.abs(company.eps_start_of_year)
        }

        target_ratio : f64 = 0.50
        
        if company.years_active < 5 { target_ratio = 0.0 }
        else if company.years_active < 15 { target_ratio = 0.15 }
